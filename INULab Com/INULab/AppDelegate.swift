...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    var window: UIWindow?
    var schCal: [SchCalendar] = []
    var labCal: [LabCalendar] = []
    var labInfos: [LabInfo] = []
    var myLab: LabInfo?
...
extension AppDelegate: NetworkCallback {
    func networkSuc(resultdata: Any, code: String, tag: Int) {
        if code == "SCH" {
            var temp: [SchCalendar] = []
            if let items = resultdata as? [NSDictionary] {
                for item in items {
                    let schName = item["schName"] as? String ?? ""
                    ...
                    let obj = SchCalendar.init(schName: schName, eventName: eventNum, date: date!, startTime: startTime, content: content)
                    //                    num += 1
                    temp.append(obj)
                }
            }
